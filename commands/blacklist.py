...
async def blacklist(interaction: discord.Interaction, role: Optional[discord.Role]):
    ...
    if not blacklistdb.is_blacklist:
        ...
        await interaction.response.send_message(f"You are currently using a whitelist! Use </whitelist:{command_whitelist.id}> instead, or you can </switchlist:{command_switchlist.id}> to switch to a blacklist.")
        return
